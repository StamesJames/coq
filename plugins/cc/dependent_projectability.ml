...
let is_field_i_dependent env sigma cnstr i : bool = 
  ...
  Feedback.msg_debug Pp.(str "+ IS FIELD I DEPENDENT +");
  Feedback.msg_debug Pp.(str "++++++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "cnstr:\n" ++ Printer.pr_constructor env (fst cnstr));
  Feedback.msg_debug Pp.(str "i:\n" ++ int i);
  Feedback.msg_debug Pp.(str "constructor_type:\n" ++ print env sigma constructor_type);
  Feedback.msg_debug Pp.(str "ind_n_params:\n" ++ int ind_n_params);
  Feedback.msg_debug Pp.(str "field_type:\n" ++ print (List.fold_left (fun env annot -> Termops.push_rel_assum annot env) env field_rel_context) sigma field_type);
...
let find_type_composition 
  ...
  Feedback.msg_debug Pp.(str "+++++++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "+ FIND TYPE COMPOSITION +");
  Feedback.msg_debug Pp.(str "+++++++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "cnstr:\n\t" ++ Printer.pr_constructor env (fst cnstr));
  Feedback.msg_debug Pp.(str "argindex:\n\t" ++ int argindex);
  Feedback.msg_debug Pp.(str "field_type:\n\t" ++ print env sigma env_field_type);
  Feedback.msg_debug Pp.(str "ind:\n\t" ++ Printer.pr_inductive env (fst ind));
  Feedback.msg_debug Pp.(str "ind_params:\n\t" ++ seq (List.map (fun e -> print env sigma e ++ str ", ") (Array.to_list env_ind_params)));
  Feedback.msg_debug Pp.(str "ind_args:\n\t" ++ seq (List.map (fun e -> print env sigma e ++ str ", ") (Array.to_list env_ind_args)));

  Feedback.msg_debug Pp.(str "constructor_type:\n\t" ++ print env sigma rel_type_of_constructor);
  Feedback.msg_debug Pp.(str "rel_field_type:\n\t" ++ print rel_target_env sigma rel_field_type_lifted);
  Feedback.msg_debug Pp.(str "rel_ind_params:\n\t" ++ seq (List.map (fun e -> print rel_target_env sigma e ++ str ", ") (Array.to_list rel_ind_params)));
  Feedback.msg_debug Pp.(str "rel_ind_args:\n\t" ++ seq (List.map (fun e -> print rel_target_env sigma e ++ str ", ") (Array.to_list rel_ind_args)));
  let rec find_type_composition_rec env sigma (rel_term_to_compose:EConstr.t) (env_term_to_compose:EConstr.t)=
    ...
    | _ ->(
      ...
      | Some (i, rel_term_to_compose_from, extraction) -> (sigma, Some (FromParameter (env_term_to_compose, env_ind_params.(i), i, extraction)))
      | None -> (
...
let projectability_test 
  ...
  if dependent then (
    ...
    Feedback.msg_debug Pp.(str "SIMPLE");
    (sigma, Simple)
  )

let rec build_extraction env sigma default rel_term_to_extract_from env_term_to_extract_from extraction =
  Feedback.msg_debug Pp.(str "++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "+ BUILD EXTRACTION +");
  Feedback.msg_debug Pp.(str "++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "extraction:\n\t" ++ pr_type_extraction env sigma extraction);
  Feedback.msg_debug Pp.(str "rel_term_to_extract_from:\n\t" ++ print env sigma rel_term_to_extract_from);
  Feedback.msg_debug Pp.(str "env_term_to_extract_from:\n\t" ++ print env sigma env_term_to_extract_from);
...
let rec build_type_composition env sigma type_composition ind_params n_ind_params ind_args n_ind_args = 
  Feedback.msg_debug Pp.(str "+++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "+ BUILD RETURN TYPE +");
  Feedback.msg_debug Pp.(str "+++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "type_composition:\n\t" ++ pr_type_composition env sigma type_composition);
  Feedback.msg_debug Pp.(str "ind_params:\n\t" ++ seq (List.map (fun e -> print env sigma e ++ str ", ") (Array.to_list ind_params)));
  Feedback.msg_debug Pp.(str "n_ind_params:\n\t" ++ int n_ind_params);
  Feedback.msg_debug Pp.(str "ind_args:\n\t" ++ seq (List.map (fun e -> print env sigma e ++ str ", ") (Array.to_list ind_args)));
  Feedback.msg_debug Pp.(str "n_ind_args:\n\t" ++ int n_ind_args);
  match type_composition with
  (* type to compose *)
...
let match_indices env sigma (cnst_summary:Inductiveops.constructor_summary) term_to_match n_ind_args =
  let rec match_indices_rec i term_to_match =
    if i > n_ind_args then term_to_match
    else Termops.replace_term sigma (EConstr.mkRel (n_ind_args-(i-1))) (cnst_summary.cs_concl_realargs.(i-1)) (match_indices_rec (i+1) term_to_match) 
  in 
    match_indices_rec 1 term_to_match
...
let make_selector_match_indices env sigma ~pos ~special c (ind_fam, ind_args) return_type composition_type_template =
  ...
  let build_branch i =
    let endpt = if Int.equal i pos then
      let matched_template = match_indices env sigma cstrs.(i-1) composition_type_template n_ind_args in
...
let build_dependent_projection env sigma prf cnstr n_cnstr_args argindex field_type lhs rhs p ti tj default special argty type_composition ((ind, ind_params) as ind_fam) ind_args: (Evd.evar_map * EConstr.t) =
  Feedback.msg_debug Pp.(str "++++++++++++++++++++++++++++++");
  ...
  Feedback.msg_debug Pp.(str "return_type_template: " ++ print env sigma composition_type_template);
  ...
  Feedback.msg_debug Pp.(str "return_type: " ++ print env sigma return_type);
  ...
  Feedback.msg_debug Pp.(str "proj:\n" ++ print env sigma proj);
  (sigma, proj)

let build_projection 
  (env : Environ.env) 
  (sigma : Evd.evar_map) 
  prf (*proof (as Cctac.proof) over wich to f_equal*)
  (cnstr : Constr.pconstructor) (*constructor that is getting projected*)
  (n_cnstr_args : int) (*number of realargs of the constructor (so without induction params)*)
  ...
  lhs (*???*)
  rhs (*???*)
  p (*goal proof (as Cctac.proof) (p.p_lhs is the left projected term)*)
  (ti : EConstr.t) (*left side of prf as EConstr*)
  (tj : EConstr.t) (*right side of prf as EConstr*)
  ...
  Feedback.msg_debug Pp.(str "++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "+ BUILD PROJECTION +");
  Feedback.msg_debug Pp.(str "++++++++++++++++++++");
  Feedback.msg_debug Pp.(str "cnstr:\n\t" ++ Printer.pr_constructor env (fst cnstr));
  Feedback.msg_debug Pp.(str "nargs:\n\t" ++ int n_cnstr_args);
  Feedback.msg_debug Pp.(str "argindex:\n\t" ++ int argindex);
  Feedback.msg_debug Pp.(str "field_type:\n\t" ++ print env sigma field_type);
  Feedback.msg_debug Pp.(str "ti:\n\t" ++ print env sigma ti);
  Feedback.msg_debug Pp.(str "tj:\n\t" ++ print env sigma tj);
  Feedback.msg_debug Pp.(str "default:\n\t" ++ print env sigma default);
  Feedback.msg_debug Pp.(str "special:\n\t" ++ print Environ.empty_env sigma default);
  Feedback.msg_debug Pp.(str "argty:\n\t" ++ print env sigma argty);
  Feedback.msg_debug Pp.(str "ind_params:\n\t" ++ seq (List.map (fun e -> print env sigma e ++ str ", ") ind_params));
  Feedback.msg_debug Pp.(str "ind_args:\n\t" ++ seq (List.map (fun e -> print env sigma e ++ str ", ") ind_args));
  ...
  | Dependent type_composition -> build_dependent_projection env sigma prf cnstr n_cnstr_args argindex field_type lhs rhs p ti tj default special argty type_composition (ind, ind_params) ind_args
